...
def main(sc):
    ...
    mentionRDD = sc.textFile('s3a://gdelt-open-data/v2/mentions/201[7-8]*00*00.mentions.csv')
    ...
    theme_file = 'list-of-top-themes-2015to9.csv'
    ...
    gkgRDD = sc.textFile('s3a://gdelt-open-data/v2/gkg/201[7-8]*00*00.gkg.csv')
    ...
    histDF.drop('tones')
    #histDF.show()
    finalDF = histDF.select('theme', 'num_mentions', 'avg', 'quantiles', 'bin_vals', col('mention_time_date').alias('time')).filter(col('theme').isin(*(rdd_theme_list.value)))
    finalDF.show()
